...
if [[ "${PROJECT_TYPE}" == "MAVEN" ]]; then
    ...
    if [[ "${CI}" == "CONCOURSE" ]]; then
        ./gradlew clean build deploy -PnewVersion=${PIPELINE_VERSION} -DM2_LOCAL=${M2_LOCAL} -DREPO_WITH_JARS=${REPO_WITH_JARS} -x test --stacktrace  || ( $( printTestResults ) && return 1)
    else
        ./gradlew clean build deploy -PnewVersion=${PIPELINE_VERSION} -DM2_LOCAL=${M2_LOCAL} -DREPO_WITH_JARS=${REPO_WITH_JARS} -x test --stacktrace
    fi
else
